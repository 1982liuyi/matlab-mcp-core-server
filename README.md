...
| ------------- | ------------- | ------------- |
| matlab-root | Full path specifying which MATLAB to start. Do not include `/bin` in the path. By default, the server tries to find the first MATLAB on the system PATH. | Windows: `"--matlab-root=C:\\Program Files\\MATLAB\\R2025b"` <br><br> Linux/macOS: `"--matlab-root=/home/usr/MATLAB/R2025b"` |
...
| matlab-session-mode | Specify whether the server starts a new MATLAB or connects to a MATLAB that is already running (supported for MATLAB R2023a onwards). To start a new MATLAB, use `new` mode (default).<br><br>To connect to a running MATLAB, use `existing` mode:<br><br><ol><li>Download [MATLAB MCP Core Server Toolbox](https://github.com/matlab/matlab-mcp-core-server/releases/latest/download/MATLABMCPCoreServerToolbox.mltbx).</li><li>Install the toolbox by double-clicking the downloaded `MATLABMCPCoreServerToolbox.mltbx` file.</li><li>In your MATLAB command window, run `shareMATLABSession()`. The MCP server connects to this MATLAB when you run the server with `--matlab-session-mode=existing`. If you are running multiple MATLAB sessions, the server connects to the MATLAB session where you most recently ran the command `shareMATLABSession()`.<br><br>As an alternative to running `shareMATLABSession()` manually, you can add the command to your MATLAB [Startup Script (MathWorks)](https://www.mathworks.com/help/matlab/ref/startup.html).</li></ol> | `"--matlab-session-mode=existing"` |
